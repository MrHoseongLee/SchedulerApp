...
class SimpleWeeklyViewRVAdapter : RecyclerView.Adapter<SimpleWeeklyViewRVAdapter.Holder>() {
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        if(assignmentsForCurrentPos !== null) {
            for(assignment in assignmentsForCurrentPos) {
                ...
                button.width = width
                button.height = (12f * assignment.estimatedTime.time * xdpi / 25.4f / 60f).toInt()
        ...
        if(meetingForCurrentPos !== null) {
            for(meeting in meetingForCurrentPos) {
                val button = Button(ContextThemeWrapper(holder.itemView.rootView.context, R.style.TaskButton))
                button.width = width
                button.height = (12.5f * (meeting.endTime.time - meeting.startTime.time) * xdpi / 25.4f).toInt()
                button.translationY = (12f * (meeting.startTime.time - startTime) / 60f * xdpi / 25.4f)
